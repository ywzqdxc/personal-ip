  ...
  return (
    <main className="min-h-screen bg-black pt-24 pb-20 px-6">
      <div className="max-w-3xl mx-auto">
        ...
        {article.category && <span className="text-xs text-gray-500 font-mono">{article.category}</span>}
        {article.summary && <p className="text-gray-300 text-lg mt-4 mb-6">{article.summary}</p>}
        {article.tags && (
          <div className="flex gap-2 mb-8">
            {article.tags.split(',').map(t => (
              <span key={t} className="text-xs px-2 py-0.5 rounded bg-white/10 text-gray-400">{t.trim()}</span>
            ))}
          </div>
        )}
        {article.content && (
          <pre className="whitespace-pre-wrap text-gray-300 text-sm leading-relaxed">{article.content}</pre>
        )}
      </div>
    </main>
  )
