    ...
    >
      ...
      <div style={{ display: 'flex', gap: 14, fontSize: 12, color: '#B07050', alignItems: 'center' }}>
        <span
          ...
          className={likeBounce ? 'icon-bounce' : ''}
        >
        ...
        <span
          ...
          style={{ cursor: 'pointer', userSelect: 'none', display: 'inline-flex', alignItems: 'center', gap: 3 }}
          className={commentBounce ? 'icon-bounce' : ''}
        >
          💬 {comments}
        </span>
      </div>
  ...
  return (
    <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', margin: '8px 0' }}>
      {tags.split(',').map((tag) => (
        <span
          key={tag.trim()}
          className={darkMode ? 'thought-tag-dark' : 'thought-tag'}
          style={{
            fontSize: 10,
    ...
    >
      {thought.imageUrl && (
        <img
          ...
          onClick={() => {
            if (imgRef.current) onImageClick?.(thought.imageUrl!, imgRef.current)
          }}
          style={{
            width: '100%',
            height: 'auto',
      ...
      <div style={{ padding: '14px 16px 16px' }}>
        ...
        {emoji && thought.mood && (
          <span style={{ fontSize: 12, color: '#B07050' }}>
            {emoji} {thought.mood}
          </span>
        )}
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '14px 14px 16px' }}>
        ...
        {/* Interaction bar in code mode — time at bottom-right */}
        <div
          style={{
            display: 'flex',
            alignItems: 'center',
...
function EssayCard({
  ...
  return (
    ...
    >
      ...
      {paragraphs.map((para, i) => (
        <p
          ...
          style={{
            ...
            ...(i === 0
              ? { borderLeft: '3px solid #E8855A', paddingLeft: 12 }
              : {}),
          }}
        >
          {para}
      ...
      {emoji && thought.mood && (
        <span style={{ fontSize: 12, color: '#B07050', display: 'inline-block', marginTop: 4 }}>
          {emoji} {thought.mood}
        </span>
      )}
...
export function Lightbox({
  ...
  // Lock scroll via Lenis while open
  useEffect(() => {
  ...
  const handleClose = useCallback(() => {
    if (phase === 'open') setPhase('exiting')
  }, [phase])
  ...
  useEffect(() => {
    if (phase === 'exiting') {
      ...
      const t = setTimeout(finishExit, 400) // safety timeout
      return () => clearTimeout(t)
    }
  }, [phase, finishExit])
  ...
  // Target: fit image within 90vw × 85vh, centered
  const maxW = vw * 0.9
  const maxH = vh * 0.85
  const scaleW = maxW / srcRect.width
  const scaleH = maxH / srcRect.height
  ...
  const isAtSource = phase === 'closed' || phase === 'entering' // start from source
  const isAtTarget = phase === 'open' || (phase === 'entering' && !isAnimating)

  // Compute transform: if entering or open → target position; if exiting → source position
  ...
  return (
    <>
      ...
      <div
        ...
        className={
          phase === 'entering' || phase === 'open'
            ? 'thought-lightbox-overlay-open'
            : 'thought-lightbox-overlay-exit'
        }
      />
      ...
      {/* Inline overlay style — handles bg fade in/out */}
      <style>{`
        .thought-lightbox-overlay-open { background: rgba(0,0,0,0.88) !important; }
        .thought-lightbox-overlay-exit { background: rgba(0,0,0,0) !important; opacity: 0; }
      `}</style>
    </>
  )
}
...
export function ThoughtCard({
  ...
  const style: React.CSSProperties = enterDelay > 0
  ...
  return (
    <div className="card-enter" style={style} data-thought-id={thought.id}>
      {type === 'photo' && (
        <PhotoCard thought={thought} emoji={emoji} onImageClick={onImageClick} />
      )}
      {type === 'code' && (
        <CodeCard thought={thought} emoji={emoji} />
      )}
      {type === 'essay' && (
        <EssayCard thought={thought} emoji={emoji} />
      )}
      {type === 'standard' && (
        <StandardCard thought={thought} emoji={emoji} />
      )}
    </div>
  )
}
